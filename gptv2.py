...
# Hyperparameters.
# Affects memory.
batch_size = 192  # Change for GPU. (4 test, 128 train)
block_size = 96  # Change for GPU. (v1 8 test, 64 train) - (v2 32 test, x train)
# Does not affect memory.
max_iterations = 10000  # Change for GPU. (v1 1000 test, 3000 train) - (v2 200 test, x train)
...
# Affect memory.
n_embed = 512  # Amount of neurons in the embedding layer.
n_head = 16  # Amount of heads (in parallel). (v1 4 for mps 8 for cuda) - (v2 1 test)
n_layer = 16  # Amount of layers (equal to number of decoder blocks). (v1 4 for mps 8 for cuda) - (v2 1 test)
# Does not affect memory.
